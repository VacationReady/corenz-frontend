...
export default function ExitInterviewFormsPage() {
  ...
  const handleDeleteTemplate = async (templateId: string, templateName: string) => {
    ...
    try {
      const response = await fetch(`/api/exit-interview-templates/${templateId}`, {
        method: 'DELETE'
      })
      ...
      {templates.length === 0 ? (
        <Card className="text-center p-8">
          <CardContent>
            ...
            <h3 className="text-lg font-semibold mb-2">No exit interview form templates yet</h3>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-6 text-sm text-gray-500">
                      ...
                      <span>{template._count.offboardings} offboarding{template._count.offboardings !== 1 ? 's' : ''}</span>
                      <span>{template._count.submissions} submission{template._count.submissions !== 1 ? 's' : ''}</span>
                    </div>
                  </div>
