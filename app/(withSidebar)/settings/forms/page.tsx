...
export default function FormsPage() {
  ...
  const filteredForms = useMemo(() => {
    return forms
      .filter(f => !query || f.name.toLowerCase().includes(query.toLowerCase()) || (f.description || '').toLowerCase().includes(query.toLowerCase()))
      .filter(f => status === 'all' ? true : status === 'active' ? f.isActive : !f.isActive)
      .filter(f => audience === 'all' ? true : audience === 'hasRoles' ? (f.visibleToRoles && f.visibleToRoles.length > 0) : !(f.visibleToRoles && f.visibleToRoles.length > 0))
      .filter(f => !recentOnly || (Date.now() - new Date(f.createdAt).getTime()) / (1000 * 60 * 60 * 24) <= 30)
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 grid grid-cols-1 md:grid-cols-4 gap-2">
        ...
        <select className="border rounded px-3 py-2" value={audience} onChange={(e) => setAudience(e.target.value as any)}>
        ...
        <Button variant="outline" onClick={() => {
          ...
          input.onchange = async () => {
            ...
            try {
              ...
              const res = await fetch('/api/forms/import', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(json) });
      ...
      {/* Preview Modal */}
      {previewForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setPreviewForm(null)}>
          <div className="bg-white rounded shadow max-w-2xl w-full p-4 m-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-lg font-semibold">{previewForm.name} – Schema Preview</h3>
              <Button variant="ghost" onClick={() => setPreviewForm(null)}>Close</Button>
            </div>
            <pre className="text-xs overflow-auto max-h-[60vh] bg-gray-50 p-3 rounded">{JSON.stringify((forms.find(x => x.id === previewForm.id) as any)?.schema ?? {}, null, 2)}</pre>
          </div>
        </div>
      )}
    </PageShell>
