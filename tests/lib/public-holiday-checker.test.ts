/**
 * Unit tests for public holiday detection
 * 
 * These tests verify the function's behavior and error handling.
 * Without a database connection, the function should gracefully return false.
 * 
 * For actual holiday validation, see public-holiday-checker.integration.test.ts
 */
 ...
 * Without database, function should return false but never throw
 */
...
test("returns false gracefully when database unavailable (Waitangi Day)", async () => {
  ...
  assert.strictEqual(typeof isHoliday, 'boolean', 'Should return boolean');
  assert.strictEqual(isHoliday, false, 'Should return false when DB unavailable');
});

test("returns false gracefully when database unavailable (ANZAC Day)", async () => {
  const anzacDay2024 = new Date('2024-04-25');
  const isHoliday = await isNZPublicHoliday(anzacDay2024, 'test-company-nz');
  assert.strictEqual(isHoliday, false, 'Should return false when DB unavailable');
});

test("returns false gracefully when database unavailable (Christmas)", async () => {
...
test("accepts region override parameter without errors", async () => {
  ...
  assert.strictEqual(typeof isHoliday, 'boolean', 'Should return boolean with region override');
});
...
test("handles leap year dates without errors", async () => {
  const leapDay2024 = new Date('2024-02-29');
  const isHoliday = await isNZPublicHoliday(leapDay2024, 'test-company-nz');
  assert.strictEqual(isHoliday, false, 'Should handle leap year gracefully');
});
...
test("handles multiple years without errors", async () => {
  ...
  assert.strictEqual(typeof result2023, 'boolean', 'Should work for 2023');
  assert.strictEqual(typeof result2024, 'boolean', 'Should work for 2024');
  assert.strictEqual(typeof result2025, 'boolean', 'Should work for 2025');
});
