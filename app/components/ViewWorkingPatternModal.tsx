...
function getPatternTypeBadge(type?: string) {
  switch (type) {
    case "SHIFT_BASED":
      return { color: "bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-500/20 dark:text-purple-300 dark:border-purple-500/30", label: "Shift-Based" };
    case "FLEXIBLE":
      return { color: "bg-orange-100 text-orange-700 border-orange-200 dark:bg-orange-500/20 dark:text-orange-300 dark:border-orange-500/30", label: "Flexible" };
    case "COMPRESSED":
      return { color: "bg-cyan-100 text-cyan-700 border-cyan-200 dark:bg-cyan-500/20 dark:text-cyan-300 dark:border-cyan-500/30", label: "Compressed" };
    default:
      return { color: "bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-500/20 dark:text-slate-300 dark:border-slate-500/30", label: "Standard" };
  }
}
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-xl overflow-hidden bg-gradient-to-br from-slate-50 via-white to-slate-100 border-slate-200 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 dark:border-white/10">
          {/* Decorative background elements */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute -top-24 -right-24 w-64 h-64 bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 rounded-full blur-3xl" />
            <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-gradient-to-tr from-violet-500/20 to-pink-500/20 rounded-full blur-3xl" />
          ...
          <div className="relative z-10">
            <DialogHeader className="pb-4 border-b border-slate-200 dark:border-white/10">
              <div className="flex items-start gap-4">
                <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 border border-emerald-200 dark:border-white/10 flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    {pattern.contractedHoursPerWeek && (
                      <span className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider rounded-full px-3 py-1 bg-slate-100 text-slate-600 border border-slate-200 dark:bg-white/5 dark:text-white/70 dark:border-white/10">
            ...
            <div className="mt-6 space-y-6 max-h-[60vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-white/10">
              {/* Description */}
              {pattern.description && (
                <div className="bg-slate-100 rounded-xl p-4 border border-slate-200 dark:bg-white/5 dark:border-white/5">
              ...
              {pattern.patternType === "SHIFT_BASED" ? (
                <motion.div
                  ...
                  className="bg-gradient-to-br from-purple-100 to-pink-100 rounded-xl p-6 border border-purple-200 dark:from-purple-500/10 dark:to-pink-500/10 dark:border-purple-500/20"
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-lg bg-purple-200 dark:bg-purple-500/20 flex items-center justify-center">
                      <Clock className="h-5 w-5 text-purple-600 dark:text-purple-400" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-slate-900 dark:text-white">Flexible Scheduling</h3>
                      <p className="text-xs text-slate-500 dark:text-white/50">Shifts created manually</p>
                    </div>
                  </div>
                  <p className="text-sm text-slate-600 dark:text-white/70">
                    This is a flexible shift-based pattern with <span className="font-semibold text-purple-700 dark:text-purple-300">{pattern.contractedHoursPerWeek || 0} contracted hours</span> per week. 
                  ...
                  {pattern.weeks.map((week, weekIndex) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden dark:bg-white/5 dark:border-white/10"
                      >
                        {/* Week Header */}
                        <div className="px-4 py-3 bg-slate-100 border-b border-slate-200 flex items-center justify-between dark:bg-white/5 dark:border-white/5">
                          <div className="flex items-center gap-2">
                            <span className="w-6 h-6 rounded-md bg-gradient-to-br from-emerald-500 to-cyan-500 flex items-center justify-center text-xs font-bold text-white">
                              {week.weekNumber}
                            </span>
                          ...
                          <span className="text-xs text-slate-500 dark:text-white/50 flex items-center gap-1.5">
                        ...
                        <div className="p-4">
                          <div className="grid grid-cols-7 gap-1 mb-3">
                            {shortDays.map((day) => (
                              <div key={day} className="text-center text-[10px] font-semibold text-slate-500 dark:text-white/40 uppercase tracking-wider">
                                {day}
                              </div>
                            ))}
                          </div>
                                ...
                                <motion.div
                                  ...
                                  className={cn(
                                    ...
                                    isWorkDay
                                      ...
                                      : "bg-slate-100 border border-slate-200 dark:bg-white/[0.02] dark:border-white/5"
                                  )}
                                >
                                  {isWorkDay ? (
                                    <>
