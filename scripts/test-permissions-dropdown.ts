...
async function testPermissionsDropdown() {
  ...
  try {
    ...
    apiResponse.profiles.forEach((profile, index) => {
      console.log(`${index + 1}. ${profile.name} (${profile.builtIn ? 'Built-in' : 'Custom'}) - ${profile.description || 'No description'}`);
    });
    ...
    const correctProfiles = Array.isArray(apiResponse.profiles) ? apiResponse.profiles : [];
    console.log(`   Array.isArray(apiResponse.profiles): ${Array.isArray(apiResponse.profiles)}`);
    console.log(`   Would set permissionProfiles to: ${correctProfiles.length} profiles`);

    console.log(`\n🔧 FIX: Change line 92 in AddEmployeeModal from:`);
    console.log(`   setPermissionProfiles(Array.isArray(profilesRes) ? profilesRes : []);`);
    console.log(`   to:`);
    console.log(`   setPermissionProfiles(Array.isArray(profilesRes.profiles) ? profilesRes.profiles : []);`);
