...
async function verifyChanges() {
  ...
  try {
    ...
    if (workingPattern) {
      ...
      console.log(`   Days: ${workingPattern.WorkingPatternWeek[0]?.WorkingPatternDay.length || 0}`);
    } else {
    ...
    if (adminProfile) {
      ...
      const employeeScreens = Object.keys(permissions).filter(key => key.startsWith('employee-'));

      console.log(`✅ Found ${employeeScreens.length} employee detail screens in Admin permissions:`);
      ...
      const missingScreens = expectedScreens.filter(screen => !employeeScreens.includes(screen));
    ...
    users.forEach(user => {
      const expectedProfile = user.role === 'ADMIN' ? 'Admin' :
      ...
      if (actualProfile === expectedProfile) {
        ...
        console.log(`❌ ${user.email}: ${user.role} → ${actualProfile} (expected ${expectedProfile})`);
      }
    });
    ...
    const employeeDetailScreens = availableScreens.filter(screen => screen.startsWith('employee-'));

    console.log(`✅ Found ${employeeDetailScreens.length} employee detail screens in permissions.ts:`);
