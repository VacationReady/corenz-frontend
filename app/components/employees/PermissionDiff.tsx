...
import { getScreenDisplayName, getActionDisplayName, PermissionAction, ScreenPermissions } from '@/lib/permissions';
...
export function PermissionDiff({ oldPermissions, newPermissions }: PermissionDiffProps) {
  const getPermissionChanges = () => {
    ...
    for (const screen of allScreens) {
      ...
      const unchanged = newActions.filter(action => oldActions.includes(action));
      ...
      {changes.map(({ screen, added, removed, unchanged }) => (
        <Card key={screen} className="border-l-4 border-l-blue-500">
          <CardHeader className="pb-3">
            <CardTitle className="text-base">{getScreenDisplayName(screen)}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            ...
            {added.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {added.map(action => (
                  <Badge key={`added-${action}`} variant="default" className="bg-green-100 text-green-800 border-green-200">
            ...
            {removed.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {removed.map(action => (
                  <Badge key={`removed-${action}`} variant="destructive" className="bg-red-100 text-red-800 border-red-200">
            ...
            {unchanged.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {unchanged.map(action => (
                  <Badge key={`unchanged-${action}`} variant="secondary" className="bg-gray-100 text-gray-700">
