...
(Module as any)._load = function (request: string, parent: any, isMain: boolean) {
  ...
  if (request.includes('ui/')) {
    // Mock UI components
    return {
      default: ({ children, ...props }: any) => ({ type: 'mock-component', props, children }),
      PageShell: ({ children, ...props }: any) => ({ type: 'PageShell', props, children }),
      Card: ({ children, ...props }: any) => ({ type: 'Card', props, children }),
      CardContent: ({ children, ...props }: any) => ({ type: 'CardContent', props, children }),
      CardHeader: ({ children, ...props }: any) => ({ type: 'CardHeader', props, children }),
      CardTitle: ({ children, ...props }: any) => ({ type: 'CardTitle', props, children }),
      CardDescription: ({ children, ...props }: any) => ({ type: 'CardDescription', props, children }),
      ...
      Select: ({ children, ...props }: any) => ({ type: 'Select', props, children }),
      SelectContent: ({ children, ...props }: any) => ({ type: 'SelectContent', props, children }),
      SelectItem: ({ children, ...props }: any) => ({ type: 'SelectItem', props, children }),
      SelectTrigger: ({ children, ...props }: any) => ({ type: 'SelectTrigger', props, children }),
      SelectValue: (props: any) => ({ type: 'SelectValue', props }),
      Badge: ({ children, ...props }: any) => ({ type: 'Badge', props, children }),
      Tabs: ({ children, ...props }: any) => ({ type: 'Tabs', props, children }),
      TabsContent: ({ children, ...props }: any) => ({ type: 'TabsContent', props, children }),
      TabsList: ({ children, ...props }: any) => ({ type: 'TabsList', props, children }),
      TabsTrigger: ({ children, ...props }: any) => ({ type: 'TabsTrigger', props, children }),
      Dialog: ({ children, ...props }: any) => ({ type: 'Dialog', props, children }),
      DialogContent: ({ children, ...props }: any) => ({ type: 'DialogContent', props, children }),
      DialogHeader: ({ children, ...props }: any) => ({ type: 'DialogHeader', props, children }),
      DialogTitle: ({ children, ...props }: any) => ({ type: 'DialogTitle', props, children }),
      DialogDescription: ({ children, ...props }: any) => ({ type: 'DialogDescription', props, children }),
      DialogTrigger: ({ children, ...props }: any) => ({ type: 'DialogTrigger', props, children }),
...
test('Automation Rules Component Tests', async (t) => {
  await t.test('AutomationRulesPage renders correctly', async () => {
    ...
    const AutomationRulesPageModule = await import('../../app/(withSidebar)/settings/automation-rules/page')
    ...
    assert.strictEqual(component.props.description, 'Create and manage no-code automation rules to streamline HR processes')
    ...
    assert.ok(actionButton.props.children.some((child: any) => 
      child.type === 'Icon' || (typeof child === 'string' && child === 'Create Rule')
    ))
  })
  ...
  await t.test('trigger type configuration', async () => {
    ...
    const triggerTypes = [
      {
        ...
        configFields: [
          { key: 'daysBefore', label: 'Days Before Expiry', type: 'number', required: true }
        ]
      },
      {
        id: 'FORM_SUBMITTED',
    ...
    const documentExpiryTrigger = triggerTypes.find(t => t.id === 'DOCUMENT_EXPIRING')
    ...
    const formSubmittedTrigger = triggerTypes.find(t => t.id === 'FORM_SUBMITTED')
  ...
  await t.test('action type configuration', async () => {
    const actionTypes = [
      {
        ...
        configFields: [
          ...
          { key: 'assigneeType', label: 'Assign To', type: 'select', required: true }
        ]
      },
      {
    ...
    const sendNotificationAction = actionTypes.find(a => a.id === 'send_notification')
    assert.ok(sendNotificationAction)
    assert.strictEqual(sendNotificationAction.configFields[0].type, 'multiselect')
  })
  ...
  await t.test('rule validation logic', async () => {
    ...
    const validRule = {
      ...
      actions: [
        { type: 'create_task', config: { title: 'Task', assigneeType: 'manager' } }
      ]
    }
  ...
  await t.test('dry run functionality', async () => {
    ...
    assert.ok(dryRunResult.preview[0].description.includes('5 documents expiring'))
  })
  ...
  await t.test('rule status management', async () => {
    ...
    assert.strictEqual(fetchCall.arguments[0], '/api/automation-rules/rule-123')
