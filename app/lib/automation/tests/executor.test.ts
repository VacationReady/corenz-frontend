...
test("AutomationActionExecutor", async (t) => {
  ...
  await t.test("create_task action", async (t) => {
    await t.test("validates configuration correctly", async () => {
      ...
      // Valid configuration
      const validResult = await executor.executeAction(
        "create_task",
        {
          title: "Test Task",
      ...
      const invalidResult = await executor.executeAction(
        "create_task",
        {
          assigneeType: "employee",
        },
  ...
  await t.test("start_onboarding action", async (t) => {
    await t.test("creates onboarding instance successfully", async () => {
      ...
      mockPrismaClient.onboardingStepInstance.create.mock
        .mockImplementationOnce(() =>
          Promise.resolve({ id: "step-instance-1" }),
        )
        .mock.mockImplementationOnce(() =>
          Promise.resolve({ id: "step-instance-2" }),
        );
    ...
    await t.test(
      "fails when employee already has active instance",
      async () => {
        const mockTemplate = {
          id: "template-123",
          isActive: true,
        ...
        assert.strictEqual(result.success, false);
        assert.ok(
          result.error?.includes("already has an active onboarding instance"),
        );
      },
    );
  });
