...
export default function FormBuilder({ onSave, initialData }: FormBuilderProps) {
  ...
  const [visibleToRoles, setVisibleToRoles] = useState<string[]>(
    ...
    initialData?.visibleToRoles || ['ADMIN', 'MANAGER', 'EMPLOYEE']

  );

  const [visibleToDepartments, setVisibleToDepartments] = useState<string[]>(

    initialData?.visibleToDepartments || []

  );

  const [visibleToJobRoles, setVisibleToJobRoles] = useState<string[]>(

    initialData?.visibleToJobRoles || []
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Input
              ...
              className={`font-mono text-sm ${!formSlug.trim() ? 'border-red-500 focus:ring-red-500' : ''}`}
  ...
  return (
    ...
    <div className="flex flex-col gap-4">
      ...
      <Button
        ...
        disabled={!formName.trim() || !fields.length || !visibleToRoles.length}
