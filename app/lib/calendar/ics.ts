 ...
 */
export function buildExitInterviewICS(options: ICSOptions): { filename: string; content: string; mime: 'text/calendar' } {
  const {
    uid,
    startTime,
    endTime,
  ...
  const icsLines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//CoreNZ//ExitInterview//EN',
    'CALSCALE:GREGORIAN',
    'METHOD:' + method,
    'BEGIN:VEVENT',
    'UID:' + uid,
  ...
  attendees.forEach(attendee => {
    ...
    icsLines.push(
      'ATTENDEE;CN=' + escapeICS(attendee.name) + ';ROLE=' + role + ':mailto:' + attendee.email
    );
  });
