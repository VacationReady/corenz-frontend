...
test('AutomationJobQueue', async (t) => {
  ...
  await t.test('enqueue accepts custom options', async () => {
    ...
    await queue.enqueue('rule-456', 'company-789', { test: 'data' }, {
      priority: 5,
      maxAttempts: 5,
      scheduledAt,
    })
    ...
    assert.deepStrictEqual(createCall.arguments[0].data.scheduledAt, scheduledAt)
  })
  ...
  await t.test('complete marks job as completed', async () => {
    ...
    assert.strictEqual(updateCall.arguments[0].data.status, AutomationJobStatus.COMPLETED)
  ...
  await t.test('fail schedules retry for job under max attempts', async () => {
    ...
    const queue = new AutomationJobQueue()
    await queue.fail('job-127', 'Test error', { error: 'details' }, 'worker-123')

    const updateCall = mockPrismaClient.automationJob.update.mock.calls[1]
    assert.strictEqual(updateCall.arguments[0].data.status, AutomationJobStatus.PENDING)
    assert.strictEqual(updateCall.arguments[0].data.errorMessage, 'Test error')
    assert.ok(updateCall.arguments[0].data.nextRetryAt)
  })

  await t.test('fail marks job as failed when max attempts reached', async () => {
    const mockJob = {
      id: 'job-128',
      attempts: 3,
      maxAttempts: 3,
    ...
    assert.strictEqual(updateCall.arguments[0].data.status, AutomationJobStatus.FAILED)
  ...
  await t.test('getQueueStats returns correct statistics', async () => {
    ...
    mockPrismaClient.automationJob.count
      .mock.mockImplementationOnce(() => Promise.resolve(5))  // pending
  ...
  await t.test('cleanup removes old jobs', async () => {
    ...
    assert.ok(deleteCall.arguments[0].where.status.in.includes(AutomationJobStatus.COMPLETED))
    assert.ok(deleteCall.arguments[0].where.status.in.includes(AutomationJobStatus.FAILED))
  ...
  await t.test('cancel marks job as cancelled', async () => {
    ...
    assert.strictEqual(updateCall.arguments[0].data.status, AutomationJobStatus.CANCELLED)
    assert.strictEqual(updateCall.arguments[0].data.errorMessage, 'Job cancelled manually')
  })
