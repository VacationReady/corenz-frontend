...
export async function POST(req: NextRequest) {
  try {
    ...
    if (offboarding.formTiming !== 'NOW') {
      return NextResponse.json({ error: "Manual form sending only works for NOW timing" }, { status: 400 });
    }
    ...
    console.log('Form invitation sent successfully for offboarding:', offboarding.id);
