...
async function createDefaultPermissionProfiles() {
  ...
  for (const profile of DEFAULT_PROFILES) {
    ...
    const permissions = DEFAULT_PERMISSIONS[profile.role as keyof typeof DEFAULT_PERMISSIONS];
    if (!permissions) {
      console.log(`⚠️  No default permissions found for role ${profile.role}, skipping...`);
      continue;
    }
    ...
    console.log(`✅ Created ${profile.name} permission profile (${createdProfile.id})`);
  }
