...
function DocumentsContent() {
  ...
  const documentTypeOptions: FilterOption[] = useMemo(() => {
    const types = documents.map(doc => doc.type).filter(Boolean).filter((type, i, arr) => arr.indexOf(type) === i);
    return [{ label: "All Types", value: "all" }, ...types.map(type => ({ label: type, value: type }))];
  }, [documents]);

  const categoryOptions: FilterOption[] = useMemo(() => {
    const categories = documents.map(doc => doc.category).filter(Boolean).filter((cat, i, arr) => arr.indexOf(cat) === i);
  ...
  const handleExport = () => {
    const csv = [
      ...
      ...filteredDocuments.map(doc => [
        doc.name, doc.category || "", doc.type,
  ...
  const handleUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFile(null); setName(""); setCategory(""); setRequiresAck(false); setRequireAckFromNewStarters(false);
        setUploadDepartments(["all"]); setUploadJobRoles(["all"]); setIsUploadModalOpen(false);
        fetchDocuments();
      } else toast("Failed to upload document.");
    } catch { toast("Error uploading document."); }
    setUploading(false);
  };
  ...
  const handleAcknowledge = async () => {
    ...
    } catch { toast("Error acknowledging document."); }
  };
  ...
  const handleRowClick = (doc: Document) => { setSelectedDoc(doc); setIsPreviewModalOpen(true); };
  ...
  return (
    ...
    >
      <TooltipProvider>
        ...
        {loading ? <p>Loading documents...</p> : filteredDocuments.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            {filters.search || filters.documentTypes.length > 0 || filters.categories.length > 0 || filters.departments.length > 0
              ? "No documents match your current filters." : "No documents found."}
          </div>
        ) : (
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {filteredDocuments.map((doc) => {
                const accessList = [
                  doc.canViewAdmin ? "Admin" : null, doc.canViewManager ? "Manager" : null, doc.canViewEmployee ? "Employee" : null,
                  ...doc.departments.map(d => d.name), ...doc.jobRoles.map(jr => jr.name)
                ].filter(Boolean);
                return (
                    ...
                    <TableCell>
                      <Tooltip>
                        ...
                        <TooltipContent className="text-xs max-w-xs">
                          {doc.departments.length === 0 && doc.jobRoles.length === 0 ? "All (Unrestricted)" : accessList.map((item, idx) => <div key={idx}>{item}</div>)}
                        </TooltipContent>
                      </Tooltip>
                    </TableCell>
                    ...
                    {userRole === "ADMIN" && (
                      <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu trigger={<Button size="sm" variant="ghost">⋮</Button>}>
                          ...
                          <DropdownMenuItem onClick={() => { setAckDocId(doc.id); setAckDocName(doc.name); setIsViewAckOpen(true); }}>View Acknowledgements</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleDelete(doc.id)} className="text-destructive">Delete</DropdownMenuItem>
                        </DropdownMenu>
                      </TableCell>
                    )}
                  </TableRow>
        ...
        <Dialog open={isUploadModalOpen} onOpenChange={setIsUploadModalOpen}>
          <DialogContent>
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              <div><Label htmlFor="file">File</Label><Input id="file" type="file" onChange={(e) => setFile(e.target.files?.[0] || null)} required /></div>
              <div><Label htmlFor="name">Document Name</Label><Input id="name" value={name} onChange={(e) => setName(e.target.value)} required /></div>
              ...
              <div><Label>Departments</Label><MultiSelect options={departmentsList} selected={uploadDepartments} onChange={setUploadDepartments} placeholder="Select departments..." /></div>
              <div><Label>Job Roles</Label><MultiSelect options={jobRolesList} selected={uploadJobRoles} onChange={setUploadJobRoles} placeholder="Select job roles..." /></div>
              <div className="flex items-center space-x-2"><Switch checked={requiresAck} onChange={setRequiresAck} /><Label>Requires Acknowledgement</Label></div>
              <div className="flex items-center space-x-2"><Switch checked={requireAckFromNewStarters} onChange={setRequireAckFromNewStarters} /><Label>Require Acknowledgement from New Starters</Label></div>
              <DialogFooter>
          ...
          <DialogContent>
            ...
            {selectedDoc && (
              <div className="space-y-4">
                ...
                <a href={selectedDoc.url} download={selectedDoc.name} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline block">Download</a>
                {selectedDoc.requiresAck && !acknowledged && <Button onClick={handleAcknowledge} className="w-full mt-2">Acknowledge Document</Button>}
                {selectedDoc.requiresAck && acknowledged && <p className="text-green-600 text-sm">✅ Acknowledged on {ackDate?.toLocaleDateString()}</p>}
              </div>
            )}
          </DialogContent>
        </Dialog>
        <EditAccessModal isOpen={isEditAccessOpen} onClose={() => setIsEditAccessOpen(false)} document={editingDoc} onSaved={fetchDocuments} />
        <ViewAcknowledgementsModal isOpen={isViewAckOpen} onClose={() => setIsViewAckOpen(false)} documentId={ackDocId} documentName={ackDocName} />
      </TooltipProvider>
    </PageShell>
  );
}
