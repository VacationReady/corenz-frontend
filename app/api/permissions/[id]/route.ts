...
export async function PUT(
  ...
  try {
    ...
    if (existingProfile.builtIn) {
      return NextResponse.json({ error: 'Cannot modify built-in profiles' }, { status: 403 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (existingProfile.builtIn) {
      return NextResponse.json({ error: 'Cannot delete built-in profiles' }, { status: 403 });
    }

    // Check if profile is in use
    if (existingProfile._count.users > 0) {
      return NextResponse.json({
        error: 'Cannot delete profile that is assigned to users. Remove all assignments first.',
      }, { status: 400 });
    }
