  ...
  if (loading) {
    return (
      <div className="w-full min-h-screen bg-content-panel p-8 flex items-center justify-center">
        <div className="text-foreground text-lg">Loading rota groups...</div>
      </div>
    );
  }
    ...
    <div className="w-full min-h-screen bg-content-panel">
      <div className="container mx-auto p-6 max-w-7xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-foreground mb-2">Rota Groups</h1>
          ...
          <Link
            ...
            className="px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground font-medium transition-all flex items-center gap-2 shadow-lg hover:shadow-xl"
          >
        ...
        {groups.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 px-4">
            ...
            <Link
              href="/admin/rota-groups/create"
              className="px-6 py-3 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground font-medium transition-all flex items-center gap-2"
            >
              <Plus className="w-5 h-5" />
        ...
        {groups.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {groups.map((group) => (
              <div
                key={group.id}
                className="bg-card backdrop-blur-md border border-border rounded-xl overflow-hidden hover:bg-accent transition-all duration-200 shadow-lg hover:shadow-xl"
              >
                <div className="p-6">
                  {/* Header with Icon */}
                  ...
                  <div className="flex gap-2 pt-4 border-t border-border">
                    <Link
                      ...
                      className="flex-1 px-3 py-2 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium text-center transition-all"
                    >
                      View Rota
                    </Link>
                    <Link
                      href={`/admin/rota-groups/${group.id}/edit`}
                      className="px-3 py-2 rounded-lg bg-accent hover:bg-accent/80 text-card-foreground transition-all"
                      title="Edit Group"
                    >
        ...
        <div className="mt-8 p-4 bg-primary/10 border border-primary/30 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-card-foreground font-medium mb-1">What are Rota Groups?</h4>
