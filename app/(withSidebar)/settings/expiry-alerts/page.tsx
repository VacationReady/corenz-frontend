...
export default function ExpirySettingsPage() {
  ...
  const applyPreset = (
    preset: "dl-90-60-30" | "training-30" | "documents-60",
  ) => {
  ...
  const handleBulkApply = async () => {
    ...
    try {
      ...
      // Refresh list
      const updated = await fetch("/api/expiry-rules/list").then((r) =>
        r.json(),
      );
      setRules(updated);
          ...
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <option value="dl-90-60-30">
                  Driver Licenses: 90/60/30 (Admin+Manager+Employee)
                </option>
            ...
            <div className="flex items-center gap-2 text-sm">
              <label className="flex items-center gap-1">
                <input
                  type="checkbox"
                  checked={bulkAdmin === true}
                  onChange={(e) => setBulkAdmin(e.target.checked ? true : null)}
                />{" "}
                Admin
              </label>
              <label className="flex items-center gap-1">
                <input
                  type="checkbox"
                  checked={bulkManager === true}
                  onChange={(e) =>
                    setBulkManager(e.target.checked ? true : null)
                  }
                />{" "}
                Manager
              </label>
              <label className="flex items-center gap-1">
                <input
                  type="checkbox"
                  checked={bulkEmployee === true}
                  onChange={(e) =>
                    setBulkEmployee(e.target.checked ? true : null)
                  }
                />{" "}
                Employee
              </label>
