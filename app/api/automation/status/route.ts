...
import { getSystemStatus, healthCheck, performMaintenance } from "@/lib/automation";
  ...
  try {
    ...
    switch (type) {
      ...
      case "queue":
        ...
        const queueStats = await automationJobQueue.getQueueStats(session.user.companyId);
      ...
      case "detailed":
        ...
        const companyQueueStats = await automationJobQueue.getQueueStats(session.user.companyId);
      ...
      default:
        ...
        return NextResponse.json({
          ...
          worker: {
            ...
            successRate: basicStatus.worker.metrics.jobsProcessed > 0 
              ? (basicStatus.worker.metrics.jobsSucceeded / basicStatus.worker.metrics.jobsProcessed * 100).toFixed(1) + "%"
              : "N/A",
          },
  ...
  try {
    ...
    switch (action) {
      ...
      case "queue_stats":
        ...
        const stats = await automationJobQueue.getQueueStats(session.user.companyId);
        return NextResponse.json({
          success: true,
          action: "queue_stats",
          result: stats,
