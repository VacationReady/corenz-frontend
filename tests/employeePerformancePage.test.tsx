...
test("createEmptyFormState returns blank fields", async () => {
  const mod = await loadPerformanceModule();
  const state = mod.createEmptyFormState();
...
test("buildPayloadFromForm normalises rating and goals", async () => {
  const mod = await loadPerformanceModule();
  const payload = mod.buildPayloadFromForm({
...
test("createFormStateFromReview hydrates edit state", async () => {
  const mod = await loadPerformanceModule();
  const review: mod.PerformanceReview = {
    id: "rev1",
    employeeId: "emp1",
    companyId: "c1",
    reviewerId: "u-admin",
  ...
  const state = mod.createFormStateFromReview(review);
