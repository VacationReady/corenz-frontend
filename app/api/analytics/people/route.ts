...
const querySchema = z.object({
  rangeInMonths: z
    ...
    .transform((value) => {
      ...
      const bounded = Math.min(Math.max(Math.round(parsed), 3), 24);
      return bounded;
    }),
  departmentId: z
    .string()
...
export async function GET(req: Request) {
  try {
    ...
    const activeEmployees = [] as typeof employees;
    ...
    const averageTenureMonths =
      tenureSamples > 0 ? Number((totalTenureMonths / tenureSamples).toFixed(1)) : null;
    ...
    for (const employee of employees) {
      ...
      const departmentName =
        ...
        "Unassigned";
      const deptEntry = departmentBreakdownMap.get(departmentKey) ?? {
      ...
      const locationEntry = locationBreakdownMap.get(locationKey) ?? {
      ...
      employmentTypeMap.set(
        employmentLabel,
        (employmentTypeMap.get(employmentLabel) ?? 0) + (employee.isActive ? 1 : 0),
      );
    }
    ...
    console.error("[ANALYTICS_PEOPLE_OVERVIEW]", error);
    return NextResponse.json(
      { error: "Unable to load people analytics" },
      { status: 500 },
    );
  }
}
