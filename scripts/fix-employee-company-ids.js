  ...
  try {
    ...
    for (const employee of employeesWithoutCompanyId) {
      if (employee.user.companyId) {
        console.log(
          `Updating employee ${employee.id} (${employee.user.email}) with companyId: ${employee.user.companyId}`,
        );
        ...
        console.warn(
          `⚠️  Employee ${employee.id} (${employee.user.email}) has no companyId on user either`,
        );
      }
    }
