...
const mockPrismaClient = {
  automationRule: {
    ...
    findFirst: test.mock.fn((args: any) => {
      const rule = mockDatabase.automationRules.find((r: any) => 
        r.id === args.where.id || 
        (args.where.companyId === r.companyId && args.where.isActive === r.isActive)
      )
  ...
  automationJob: {
    ...
    findFirst: test.mock.fn((args: any) => {
      const job = mockDatabase.automationJobs.find((j: any) => {
        ...
        if (args.where.scheduledAt?.lte && j.scheduledAt > args.where.scheduledAt.lte) return false
        return true
      })
      return Promise.resolve(job || null)
    }),
    update: test.mock.fn((args: any) => {
      const jobIndex = mockDatabase.automationJobs.findIndex((j: any) => j.id === args.where.id)
      if (jobIndex >= 0) {
  ...
  employmentCheck: {
    findMany: test.mock.fn((args: any) => {
      let checks = mockDatabase.employmentChecks.filter((c: any) => {
        if (args.where.employee?.companyId && c.employee.companyId !== args.where.employee.companyId) return false
        if (args.where.expiryDate?.gte && c.expiryDate < args.where.expiryDate.gte) return false
        if (args.where.expiryDate?.lte && c.expiryDate > args.where.expiryDate.lte) return false
        return true
      })
  ...
  employee: {
    findUnique: test.mock.fn((args: any) => {
      const employee = mockDatabase.employees.find((e: any) => e.id === args.where.id)
      return Promise.resolve(employee || null)
    }),
    findMany: test.mock.fn((args: any) => {
      let employees = mockDatabase.employees.filter((e: any) => {
        if (args.where.companyId && e.companyId !== args.where.companyId) return false
  ...
  user: {
    findMany: test.mock.fn((args: any) => {
      let users = mockDatabase.users.filter((u: any) => {
        if (args.where.companyId && u.companyId !== args.where.companyId) return false
...
test('Automation Workflow Integration Tests', async (t) => {
  await t.test('Complete document expiry workflow', async () => {
    ...
    assert.strictEqual(triggerResult.matchingEntities[0].type, 'document_expiry')
    ...
    assert.strictEqual(mockDatabase.automationJobs[0].status, AutomationJobStatus.PENDING)
    ...
    const completedJob = mockDatabase.automationJobs.find((j: any) => j.id === job.id)
  ...
  await t.test('Form submission workflow with conditions', async () => {
    ...
    const { AutomationScheduler } = await import('../../lib/automation/scheduler')
  ...
  await t.test('Failed job retry workflow', async () => {
    ...
    const failedJob = mockDatabase.automationJobs.find((j: any) => j.id === jobId)
    ...
    const retriedJob = mockDatabase.automationJobs.find((j: any) => j.id === jobId)
    ...
    const finalJob = mockDatabase.automationJobs.find((j: any) => j.id === jobId)
